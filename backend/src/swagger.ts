...
const ROUTES: Record<string, RouteDef[]> = {
  ...
  Employees: [
    ...
    ['post', '/employees/business/{businessId}', 'Add a team member (OWNER only)'],
    ['patch', '/employees/{id}', 'Update an employee (member; level/userId owner-only)'],
    ['delete', '/employees/{id}', 'Remove a team member (OWNER only)'],
  ],
  Users: [
    ['get', '/users', 'List users (public directory)'],
    ['get', '/users/me/is-super-admin', 'Derived super-admin flag for the caller'],
  ...
  Calls: [
    ...
    ['get', '/calls/business/{businessId}', 'Call log, last 7 days or ?since=<iso> (member)'],
  ...
  Memberships: [
    ...
    ['post', '/memberships', 'Enrol a customer (member) — customerId may be a walkin:<name> key; optional enrolleeName'],
